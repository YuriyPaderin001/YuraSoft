...
namespace YuraSoft.QueryBuilder.Common.Tests.Elements.Sources
{
	public class TableTests
	{
		...
		{
			// Arrange
			Table table = new Table(name: "test_name", alias, schema);
		...
		{
			// Arrange
			Table table = new Table(name: "test_name");

			// Act & Assert
			Assert.Throws<ArgumentException>(() => table.Name = name!);
		}
		...
		public void SetAlias_Success(string? alias, string? schema)
		{
			...
			Table table = new Table(name, "test_alias", schema);
		...
		public void SetSchema_Success(string? alias, string? schema)
		{
			// Arrange
			const string name = "test_name";
			Table table = new Table(name, alias, "test_schema");
